...
@section('description', __('contact.page_sub'))
...
<section class="bg-surface border-b border-black/6 pt-[70px]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14 text-center reveal">
        <p class="text-xs font-semibold tracking-widest uppercase text-muted mb-3">{{ __('contact.section_label') }}</p>
        <h1 class="text-3xl lg:text-5xl font-extrabold text-text mb-4">{{ __('contact.page_heading') }}</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white border border-black/6 rounded-2xl p-7 sm:p-10 shadow-sm">
                    ...
                    {{-- Success message --}}
                    @if(session('success'))
                        <div class="mb-6 p-5 rounded-xl border border-green-200 bg-green-50 flex items-start gap-3">
                            <svg class="shrink-0 mt-0.5" width="18" height="18" fill="none" stroke="#10B981" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <div>
                                <p class="text-text font-semibold text-sm">{{ __('contact.success_heading') }}</p>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white border border-black/6 rounded-2xl p-6 shadow-sm">
                    <h3 class="text-text font-semibold text-sm uppercase tracking-widest mb-5">{{ __('contact.info_heading') }}</h3>
                    <div class="space-y-4">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <a href="mailto:{{ $settings['contact_email'] }}"
                                       class="text-text text-sm font-medium hover:text-primary transition truncate block">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <a href="https://wa.me/{{ preg_replace('/\D/', '', $settings['whatsapp_number']) }}"
                                       ...
                                       class="text-text text-sm font-medium hover:text-primary transition">
                        ...
                        @if(!empty($settings['messenger_url']))
                            <div class="flex items-start gap-3">
                                <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                                     style="background:rgba(0,120,255,0.08);border:1px solid rgba(0,120,255,0.18)">
                ...
                @if(!empty($settings['facebook_url']) || !empty($settings['twitter_url']) || !empty($settings['linkedin_url']) || !empty($settings['instagram_url']))
                    <div class="bg-white border border-black/6 rounded-2xl p-6 shadow-sm">
                        <h3 class="text-text font-semibold text-sm uppercase tracking-widest mb-4">{{ __('contact.follow_heading') }}</h3>
                        <div class="flex flex-wrap gap-3">
                            @if(!empty($settings['facebook_url']))
                                <a href="{{ $settings['facebook_url'] }}" target="_blank" rel="noopener noreferrer"
                                   class="w-10 h-10 rounded-xl bg-surface border border-black/8 flex items-center justify-center text-muted hover:text-primary hover:bg-primary/8 hover:border-primary/25 transition">
