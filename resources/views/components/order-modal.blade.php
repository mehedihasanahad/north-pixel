...
@php
    ...
    $waMsg    = rawurlencode($settings['order_message_en'] ?? 'Hi! I\'m interested in ordering from your store.');
...
>
    ...
    <div id="order-modal-inner" @click.stop>
        ...
        {{-- Product info (when set) --}}
        <template x-if="$store.ui.modalProduct">
            <div class="mb-5 p-4 rounded-xl bg-white/4 border border-white/7">
                <p class="text-xs text-muted mb-1">{{ __('modal.heading') }}</p>
                <p class="font-semibold text-sm text-white" x-text="$store.ui.modalProduct.title"></p>
            </div>
        </template>

        <h2 id="modal-title" class="text-xl font-bold text-white mb-1">{{ __('modal.heading') }}</h2>
        <p class="text-muted text-sm mb-6">{{ __('modal.sub') }}</p>
