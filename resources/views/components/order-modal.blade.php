...
>
    ...
    <div id="order-modal-inner" @click.stop>
        {{-- Handle --}}
        <div class="w-10 h-1 rounded-full bg-black/10 mx-auto mb-6"></div>

        {{-- Close --}}
        <button @click="$store.ui.closeModal()"
            class="absolute top-5 right-5 p-2 rounded-lg text-muted hover:text-text hover:bg-black/5 transition"
        ...
        <template x-if="$store.ui.modalProduct">
            <div class="mb-5 p-4 rounded-xl bg-surface border border-black/6">
                <p class="text-xs text-muted mb-1"
                   x-text="$store.ui.modalProduct.isPrebook ? '{{ __('modal.heading_prebook') }}' : '{{ __('modal.heading') }}'"></p>
                <p class="font-semibold text-sm text-text" x-text="$store.ui.modalProduct.title"></p>
            </div>
        </template>

        <h2 id="modal-title" class="text-xl font-bold text-text mb-1"
        ...
        <div class="space-y-3">
            <a :href="waHref" target="_blank" rel="noopener noreferrer" class="btn-wa">
